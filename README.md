# Seerstack TypeScript API Library

[![NPM version](<https://img.shields.io/npm/v/seerstack.svg?label=npm%20(stable)>)](https://npmjs.org/package/seerstack) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/seerstack)

This library provides convenient access to the Seerstack REST API from server-side TypeScript or JavaScript.
...
```sh
npm install git+ssh://git@github.com:stainless-sdks/seerstack-typescript.git
```
...
1. Via the `SEERSTACK_LOG` environment variable
...
const client = new Seerstack({
  logger: logger.child({ name: 'Seerstack' }),
...
const client = new Seerstack({ fetch });
```
...
```ts
import Seerstack from 'npm:seerstack';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/seerstack-typescript/issues) with questions, bugs, or suggestions.
