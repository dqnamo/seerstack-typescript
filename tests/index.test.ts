...
import { APIPromise } from 'seerstack/core/api-promise';

import util from 'node:util';
import Seerstack from 'seerstack';
import { APIUserAbortError } from 'seerstack';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['SEERSTACK_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Seerstack) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Seerstack({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['SEERSTACK_LOG'] = 'debug';
      const client = new Seerstack({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['SEERSTACK_LOG'] = 'not a log level';
      const client = new Seerstack({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'SEERSTACK_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Seerstack({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['SEERSTACK_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Seerstack({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['SEERSTACK_BASE_URL'] = 'https://example.com/from_env';
      const client = new Seerstack({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['SEERSTACK_BASE_URL'] = ''; // empty
      const client = new Seerstack({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.userplex.app');
    });

    test('blank env variable', () => {
      process.env['SEERSTACK_BASE_URL'] = '  '; // blank
      const client = new Seerstack({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.userplex.app');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Seerstack({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['SEERSTACK_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Seerstack({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Seerstack({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Seerstack({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['SEERSTACK_API_KEY'] = 'My API Key';
    const client = new Seerstack();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['SEERSTACK_API_KEY'] = 'another My API Key';
    const client = new Seerstack({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Seerstack({ apiKey: 'My API Key', fetch: testFetch });
